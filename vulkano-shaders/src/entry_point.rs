...
pub fn write_entry_point(doc: &parse::Spirv, instruction: &parse::Instruction) -> (String, String) {
  ...
  let (ty, f_call) = {
    if let enums::ExecutionModel::ExecutionModelGLCompute = *execution {
      (
        ...
        format!(
          "compute_entry_point(::std::ffi::CStr::from_ptr(NAME.as_ptr() as *const _), \
           Layout(ShaderStages {{ compute: true, .. ShaderStages::none() }}))"
        ),
      )
    } else {
      let ty = match *execution {
        enums::ExecutionModel::ExecutionModelVertex => "::vulkano::pipeline::shader::GraphicsShaderType::Vertex".to_owned(),
        ...
        enums::ExecutionModel::ExecutionModelFragment => "::vulkano::pipeline::shader::GraphicsShaderType::Fragment".to_owned(),
      ...
      let stage = match *execution {
        enums::ExecutionModel::ExecutionModelVertex => "ShaderStages { vertex: true, .. ShaderStages::none() }",
        enums::ExecutionModel::ExecutionModelTessellationControl => "ShaderStages { tessellation_control: true, .. ShaderStages::none() }",
        enums::ExecutionModel::ExecutionModelTessellationEvaluation => {
          "ShaderStages { tessellation_evaluation: true, .. ShaderStages::none() }"
        }
        enums::ExecutionModel::ExecutionModelGeometry => "ShaderStages { geometry: true, .. ShaderStages::none() }",
        enums::ExecutionModel::ExecutionModelFragment => "ShaderStages { fragment: true, .. ShaderStages::none() }",
        enums::ExecutionModel::ExecutionModelGLCompute => unreachable!(),
        enums::ExecutionModel::ExecutionModelKernel => unreachable!(),
      };

      let t = format!(
        "::vulkano::pipeline::shader::GraphicsEntryPoint<{0}, {1}Input, \
         {1}Output, Layout>",
        spec_consts_struct, capitalized_ep_name
      );
      let f = format!(
        "graphics_entry_point(::std::ffi::CStr::from_ptr(NAME.as_ptr() \
  ...
  let entry_point = format!(
    ...
    ty = ty,
    encoded_ep_name = ep_name.chars().map(|c| (c as u32).to_string()).collect::<Vec<String>>().join(", "),
    f_call = f_call
  );
