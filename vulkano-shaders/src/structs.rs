...
fn write_struct(doc: &parse::Spirv, struct_id: u32, members: &[u32]) -> (String, Option<usize>) {
      ...
      parse::Instruction::Decorate {
        ...
        for inst in doc.instructions.iter() {
          match *inst {
            ...
            parse::Instruction::TypeRuntimeArray {
              result_id,
              type_id,
            }
              if result_id == target_id && type_id == struct_id =>
  ...
  (
    s,
    spirv_req_total_size.map(|sz| sz as usize).or(current_rust_offset),
  )
}
    ...
    match instruction {
        ...
        if result_id == searched =>
      {
        ...
        let stride = doc
          ...
          .next()
          .expect("failed to find ArrayStride decoration");
        if stride as usize > t_size {
          panic!(
            "Not possible to generate a rust array with the correct alignment since the SPIR-V \
