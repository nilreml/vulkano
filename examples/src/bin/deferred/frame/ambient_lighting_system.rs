...
impl AmbientLightingSystem {
    ...
    R: RenderPassAbstract + Send + Sync + 'static,
  {
    ...
    let pipeline = {
      let vs = vs::Shader::load(gfx_queue.device().clone()).expect("failed to create shader module");
      let fs = fs::Shader::load(gfx_queue.device().clone()).expect("failed to create shader module");
  ...
  ///
  pub fn draw<C>(&self, viewport_dimensions: [u32; 2], color_input: C, ambient_color: [f32; 3]) -> AutoCommandBuffer
  where
