...
impl FrameSystem {
  ...
  pub fn new(gfx_queue: Arc<Queue>, final_output_format: Format) -> FrameSystem {
    ...
    let diffuse_buffer =
      AttachmentImage::with_usage(gfx_queue.device().clone(), [1, 1], Format::A2B10G10R10UnormPack32, atch_usage).unwrap();
    let normals_buffer = AttachmentImage::with_usage(gfx_queue.device().clone(), [1, 1], Format::R16G16B16A16Sfloat, atch_usage).unwrap();
    let depth_buffer = AttachmentImage::with_usage(gfx_queue.device().clone(), [1, 1], Format::D16Unorm, atch_usage).unwrap();
    ...
    let ambient_lighting_system = AmbientLightingSystem::new(gfx_queue.clone(), lighting_subpass.clone());
    let directional_lighting_system = DirectionalLightingSystem::new(gfx_queue.clone(), lighting_subpass.clone());
  ...
  ///
  pub fn frame<F, I>(&mut self, before_future: F, final_image: I, world_to_framebuffer: Matrix4<f32>) -> Frame
  where
    F: GpuFuture + 'static,
    I: ImageAccess + ImageViewAccess + Clone + Send + Sync + 'static,
  {
    ...
    if ImageAccess::dimensions(&self.diffuse_buffer).width_height() != img_dims {
      ...
      .unwrap();
      self.normals_buffer =
        AttachmentImage::with_usage(self.gfx_queue.device().clone(), img_dims, Format::R16G16B16A16Sfloat, atch_usage).unwrap();
      self.depth_buffer = AttachmentImage::with_usage(self.gfx_queue.device().clone(), img_dims, Format::D16Unorm, atch_usage).unwrap();
    }
    ...
    let command_buffer = Some(
      AutoCommandBufferBuilder::primary_one_time_submit(self.gfx_queue.device().clone(), self.gfx_queue.family())
        .unwrap()
        .begin_render_pass(
          framebuffer.clone(),
          true,
...
impl<'f, 's: 'f> DrawPass<'f, 's> {
    ...
    C: CommandBuffer + Send + Sync + 'static,
  {
    ...
    // TODO: ^
    unsafe {
      self.frame.command_buffer = Some(self.frame.command_buffer.take().unwrap().execute_commands(command_buffer).unwrap());
    }
  }
    ...
    unsafe {
      ...
      let command_buffer =
        self
          ...
          .system
          .ambient_lighting_system
          .draw([dims[0], dims[1]], self.frame.system.diffuse_buffer.clone(), color);
      self.frame.command_buffer = Some(self.frame.command_buffer.take().unwrap().execute_commands(command_buffer).unwrap());
    }
  }
