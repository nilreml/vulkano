...
fn main() {
  let instance = vk::instance::Instance::new(None, &vulkano_win::required_extensions(), None)
  ...
  let fs = {
    let mut f =
      File::open("src/bin/runtime-shader/frag.spv").expect("Can't find file src/bin/runtime-shader/frag.spv");
    let mut v = vec![];
    f.read_to_end(&mut v).unwrap();
    unsafe { ShaderModule::new(graphics_device.clone(), &v) }.unwrap()
  };
  ...
  loop {
    ...
    let (image_num, acquire_future) = vk::swapchain::acquire_next_image(swapchain.clone(), None)
