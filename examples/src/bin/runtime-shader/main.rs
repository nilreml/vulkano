...
fn main() {
  let instance = vk::instance::Instance::new(None, &vulkano_win::required_extensions(), None).expect("no instance with surface extension");
  ...
  let fs = {
    let mut f = File::open("src/bin/runtime-shader/frag.spv").expect("Can't find file src/bin/runtime-shader/frag.spv");
    let mut v = vec![];
    f.read_to_end(&mut v).unwrap();
    unsafe { ShaderModule::new(graphics_device.clone(), &v) }.unwrap()
  };
  ...
  let framebuffers: Vec<_> = images
    .iter()
    .map(|image| Arc::new(Framebuffer::start(renderpass.clone()).add(image.clone()).unwrap().build().unwrap()))
    .collect();
  ...
  loop {
    ...
    let (image_num, acquire_future) =
      vk::swapchain::acquire_next_image(swapchain.clone(), None).expect("failed to acquire swapchain in time");

    let command_buffer = AutoCommandBufferBuilder::new(graphics_device.clone(), graphics_queue.family())
      .unwrap()
      .begin_render_pass(framebuffers[image_num].clone(), false, vec![[0.0, 0.0, 0.0, 1.0].into()])
      .unwrap()
      .draw(graphics_pipeline.clone(), &DynamicState::none(), vertex_buffer.clone(), (), ())
      .unwrap()
      .end_render_pass()
      .unwrap()
      .build()
