...
fn main() {
  ...
  let intermediary = AttachmentImage::transient_multisampled(device.clone(), [1024, 1024], 4, Format::R8G8B8A8Unorm).unwrap();
  ...
  let vertex_buffer =
    CpuAccessibleBuffer::from_iter(device.clone(), BufferUsage::all(), vec![vertex1, vertex2, vertex3].into_iter()).unwrap();
  ...
  let buf =
    CpuAccessibleBuffer::from_iter(device.clone(), BufferUsage::all(), (0..1024 * 1024 * 4).map(|_| 0u8)).expect("failed to create buffer");
  ...
  let command_buffer = AutoCommandBufferBuilder::primary_one_time_submit(device.clone(), queue.family())
    ...
    .begin_render_pass(
      framebuffer.clone(),
      false,
      vec![[0.0, 0.0, 1.0, 1.0].into(), vulkano::format::ClearValue::None],
    )
    .unwrap()
