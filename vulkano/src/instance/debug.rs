    ...
    F: Fn(&Message) + 'static + Send + panic::RefUnwindSafe,
  {
    ...
    extern "system" fn callback(
      ...
      unsafe {
        ...
        let layer_prefix = CStr::from_ptr(layer_prefix).to_str().expect("debug callback message not utf-8");
        let description = CStr::from_ptr(description).to_str().expect("debug callback message not utf-8");
    ...
    let debug_report_callback = unsafe {
      ...
      check_errors(vk.CreateDebugReportCallbackEXT(instance.internal_object(), &infos, ptr::null(), &mut output))?;
      output
    };
...
impl Drop for DebugCallback {
  ...
  fn drop(&mut self) {
    unsafe {
      ...
      vk.DestroyDebugReportCallbackEXT(self.instance.internal_object(), self.debug_report_callback, ptr::null());
    }
  }
}
