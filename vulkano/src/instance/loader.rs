...
pub unsafe trait Loader {
  ...
  fn get_instance_proc_addr(&self, instance: vk::Instance, name: *const c_char) -> extern "system" fn() -> ();
}
...
pub struct DynamicLibraryLoader {
  ...
  get_proc_addr: extern "system" fn(instance: vk::Instance, pName: *const c_char) -> extern "system" fn() -> (),
}
...
impl DynamicLibraryLoader {
  ...
  where
    P: AsRef<Path>,
  {
    let vk_lib = shared_library::dynamic_library::DynamicLibrary::open(Some(path.as_ref())).map_err(LoadingError::LibraryLoadFailure)?;
...
impl<L> FunctionPointers<L> {
  ...
  #[inline]
  pub fn get_instance_proc_addr(&self, instance: vk::Instance, name: *const c_char) -> extern "system" fn() -> ()
  where
    L: Loader,
  {
...
macro_rules! statically_linked_vulkan_loader {
  () => {{
    extern "C" {
      fn vkGetInstanceProcAddr(instance: vk::Instance, pName: *const c_char) -> vk::PFN_vkVoidFunction;
    }
