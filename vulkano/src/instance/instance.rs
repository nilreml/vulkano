...
impl Instance {
  ...
  pub fn new<'a, L, Ext>(app_infos: Option<&ApplicationInfo>, extensions: Ext, layers: L) -> Result<Arc<Instance>, InstanceCreationError>
  where
    ...
    Ext: Into<RawInstanceExtensions>,
  {
    ...
    Instance::new_inner(app_infos, extensions.into(), layers, OwnedOrRef::Ref(loader::auto_loader()?))
  }

  /// Same as `new`, but allows specifying a loader where to load Vulkan from.
  pub fn with_loader<'a, L, Ext>(
    loader: FunctionPointers<Box<Loader + Send + Sync>>,
  ...
  fn new_inner(
    ...
    let extensions_list = extensions.iter().map(|extension| extension.as_ptr()).collect::<SmallVec<[_; 32]>>();
    ...
    let vk =
      { vk::InstancePointers::load(|name| unsafe { mem::transmute(function_pointers.get_instance_proc_addr(instance, name.as_ptr())) }) };
...
impl<'a> QueueFamily<'a> {
  ...
  pub fn supports_transfers(&self) -> bool {
    (self.flags() & vk::QUEUE_TRANSFER_BIT) != 0 || self.supports_graphics() || self.supports_compute()
  }
