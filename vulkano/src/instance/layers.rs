...
pub fn layers_list_from_loader<L>(ptrs: &loader::FunctionPointers<L>) -> Result<LayersIterator, LayersListError>
where
  L: loader::Loader,
{
  unsafe {
    ...
    check_errors({ entry_points.EnumerateInstanceLayerProperties(&mut num, layers.as_mut_ptr()) })?;
...
impl Iterator for LayersIterator {
  ...
  fn next(&mut self) -> Option<LayerProperties> {
    self.iter.next().map(|p| LayerProperties {
      props: p
    })
  }
