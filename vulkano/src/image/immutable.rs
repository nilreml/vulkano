...
impl<F> ImmutableImage<F> {
  ...
  pub fn from_iter<P, I>(
    ...
    format: F,
    queue: Arc<Queue>,
  ) -> Result<(Arc<Self>, CommandBufferExecFuture<NowFuture, AutoCommandBuffer>), ImageCreationError>
  where
  ...
  {
    let source = CpuAccessibleBuffer::from_iter(queue.device().clone(), BufferUsage::transfer_source(), iter)?;
