...
impl UnsafeImage {
  ...
  unsafe fn new_impl(
    ...
    let format_features = {
      ...
      if usage.input_attachment
        && (features & (vk::FORMAT_FEATURE_COLOR_ATTACHMENT_BIT | vk::FORMAT_FEATURE_DEPTH_STENCIL_ATTACHMENT_BIT) == 0)
      {
        return Err(ImageCreationError::UnsupportedUsage);
      }
      if device.loaded_extensions().khr_maintenance1 {
        ...
        if usage.transfer_destination && (features & vk::FORMAT_FEATURE_TRANSFER_DST_BIT_KHR == 0) {
          return Err(ImageCreationError::UnsupportedUsage);
        }
      }
    ...
    if num_samples == 0 {
      ...
      if usage.sampled {
        match format.ty() {
          FormatTy::Float | FormatTy::Compressed => {
            supported_samples &= device.physical_device().limits().sampled_image_color_sample_counts();
          }
          FormatTy::Uint | FormatTy::Sint => {
            supported_samples &= device.physical_device().limits().sampled_image_integer_sample_counts();
          }
          FormatTy::Depth => {
            supported_samples &= device.physical_device().limits().sampled_image_depth_sample_counts();
          }
          FormatTy::Stencil => {
            supported_samples &= device.physical_device().limits().sampled_image_stencil_sample_counts();
          }
      ...
      if usage.storage {
        supported_samples &= device.physical_device().limits().storage_image_sample_counts();
      }
      ...
      if usage.color_attachment || usage.depth_stencil_attachment || usage.input_attachment || usage.transient_attachment {
        match format.ty() {
          FormatTy::Float | FormatTy::Compressed | FormatTy::Uint | FormatTy::Sint => {
            supported_samples &= device.physical_device().limits().framebuffer_color_sample_counts();
          }
          FormatTy::Depth => {
            supported_samples &= device.physical_device().limits().framebuffer_depth_sample_counts();
          }
          FormatTy::Stencil => {
            supported_samples &= device.physical_device().limits().framebuffer_stencil_sample_counts();
          }
          FormatTy::DepthStencil => {
            supported_samples &= device.physical_device().limits().framebuffer_depth_sample_counts();
            supported_samples &= device.physical_device().limits().framebuffer_stencil_sample_counts();
    ...
    if let Some(capabilities_error) = capabilities_error {
      ...
      let physical_device = device.physical_device().internal_object();
      let r =
        vk_i.GetPhysicalDeviceImageFormatProperties(physical_device, format as u32, ty, tiling, usage, 0 /* TODO */, &mut output);
    ...
    let image = {
      ...
      let mut output = mem::uninitialized();
      check_errors(vk.CreateImage(device.internal_object(), &infos, ptr::null(), &mut output))?;
      output
    };
  ...
  unsafe fn linear_layout_impl(&self, mip_level: u32, aspect: u32) -> LinearLayout {
    ...
    vk.GetImageSubresourceLayout(self.device.internal_object(), self.image, &subresource, &mut out);
...
impl error::Error for ImageCreationError {
  ...
  fn description(&self) -> &str {
    match *self {
      ...
      ImageCreationError::FormatNotSupported => "the requested format is not supported by the Vulkan implementation",
...
impl UnsafeImageView {
  ...
  pub unsafe fn raw(
    ...
    let view = {
      ...
      let mut output = mem::uninitialized();
      check_errors(vk.CreateImageView(image.device.internal_object(), &infos, ptr::null(), &mut output))?;
      output
    };
