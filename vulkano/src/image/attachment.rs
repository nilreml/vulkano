...
impl<F> AttachmentImage<F> {
  ...
  #[inline]
  pub fn input_attachment(device: Arc<Device>, dimensions: [u32; 2], format: F) -> Result<Arc<AttachmentImage<F>>, ImageCreationError>
  where
    F: FormatDesc,
  {
  ...
  /// > **Note**: This function is just a convenient shortcut for `with_usage`.
  #[inline]
  pub fn sampled(device: Arc<Device>, dimensions: [u32; 2], format: F) -> Result<Arc<AttachmentImage<F>>, ImageCreationError>
  where
    F: FormatDesc,
  {
    let base_usage = ImageUsage {
