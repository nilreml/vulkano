...
impl Device {
    ...
    Ext: Into<RawDeviceExtensions>,
  {
    ...
    let extensions_list = extensions.iter().map(|extension| extension.as_ptr()).collect::<SmallVec<[_; 16]>>();
    ...
    let device = unsafe {
      ...
      for (queue_family, priority) in queue_families {
        ...
        assert_eq!(queue_family.physical_device().internal_object(), phys.internal_object());
      ...
      check_errors(vk_i.CreateDevice(phys.internal_object(), &infos, ptr::null(), &mut output))?;
      output
    };
...
impl error::Error for DeviceCreationError {
  ...
  fn description(&self) -> &str {
    match *self {
      DeviceCreationError::InitializationFailed => "failed to create the device for an implementation-specific reason",
      ...
      DeviceCreationError::FeatureNotPresent => "some of the requested features are unsupported by the physical device",
      DeviceCreationError::PriorityOutOfRange => "the priority of one of the queues is out of the [0.0; 1.0] range",
      DeviceCreationError::ExtensionNotPresent => "some of the requested device extensions are not supported by the physical device",
...
impl Queue {
  ...
  pub fn is_same(&self, other: &Queue) -> bool {
    self.id == other.id && self.family == other.family && self.device.internal_object() == other.device.internal_object()
  }
...
mod tests {
  ...
  fn priority_out_of_range() {
    ...
    match Device::new(physical, &Features::none(), &DeviceExtensions::none(), Some((family, 1.4))) {
      Err(DeviceCreationError::PriorityOutOfRange) => (), // Success
      _ => panic!(),
    };

    match Device::new(physical, &Features::none(), &DeviceExtensions::none(), Some((family, -0.2))) {
      Err(DeviceCreationError::PriorityOutOfRange) => (), // Success
      _ => panic!(),
    };
  }
