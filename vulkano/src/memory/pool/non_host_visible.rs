...
impl StdNonHostVisibleMemoryTypePool {
  ...
  ///
  pub fn alloc(me: &Arc<Self>, size: usize, alignment: usize) -> Result<StdNonHostVisibleMemoryTypePoolAlloc, DeviceMemoryAllocError> {
    assert!(size != 0);
    assert!(alignment != 0);
...
impl Drop for StdNonHostVisibleMemoryTypePoolAlloc {
  fn drop(&mut self) {
    ...
    let entries = occupied.iter_mut().find(|e| &*e.0 as *const DeviceMemory == &*self.memory).unwrap();
