...
impl StdHostVisibleMemoryTypePool {
  ...
  ///
  pub fn alloc(me: &Arc<Self>, size: usize, alignment: usize) -> Result<StdHostVisibleMemoryTypePoolAlloc, DeviceMemoryAllocError> {
    assert!(size != 0);
    assert!(alignment != 0);
    ...
    let new_block = {
      ...
      let new_block = DeviceMemory::alloc_and_map(me.device.clone(), me.memory_type(), to_alloc)?;
      Arc::new(new_block)
    };
