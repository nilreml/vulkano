...
impl DeviceMemory {
  ...
  ///
  #[inline]
  pub fn alloc(device: Arc<Device>, memory_type: MemoryType, size: usize) -> Result<DeviceMemory, DeviceMemoryAllocError> {
  ...
  pub fn dedicated_alloc(
    ...
    let memory = unsafe {
      ...
      check_errors(vk.AllocateMemory(device.internal_object(), &infos, ptr::null(), &mut output))?;
      *allocation_count += 1;
      output
    };
  ...
  ///
  #[inline]
  pub fn alloc_and_map(device: Arc<Device>, memory_type: MemoryType, size: usize) -> Result<MappedDeviceMemory, DeviceMemoryAllocError> {
...
impl MappedDeviceMemory {
    ...
    T: Content,
  {
    ...
    let pointer = T::ref_from_ptr((self.pointer as usize + range.start) as *mut _, range.end - range.start).unwrap(); // TODO: error
...
impl From<Error> for DeviceMemoryAllocError {
  ...
  fn from(err: Error) -> DeviceMemoryAllocError {
    match err {
      e @ Error::OutOfHostMemory | e @ Error::OutOfDeviceMemory => DeviceMemoryAllocError::OomError(e.into()),
