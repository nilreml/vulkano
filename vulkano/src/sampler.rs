...
impl Sampler {
  ...
  fn new_impl(
    ...
    let sampler = unsafe {
      ...
      check_errors(vk.CreateSampler(device.internal_object(), &infos, ptr::null(), &mut output))?;
      output
    };
...
impl error::Error for SamplerCreationError {
  ...
  fn description(&self) -> &str {
    match *self {
      ...
      SamplerCreationError::SamplerAnisotropyFeatureNotEnabled => "the `sampler_anisotropy` feature is not enabled",
      SamplerCreationError::AnisotropyLimitExceeded {
        ..
      } => "anisotropy limit exceeded",
