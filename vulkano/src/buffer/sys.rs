...
impl UnsafeBuffer {
    ...
    I: Iterator<Item = u32>,
  {
    ...
    let buffer = {
      ...
      check_errors(vk.CreateBuffer(device.internal_object(), &infos, ptr::null(), &mut output))?;
      output
    };
    ...
    let mem_reqs = {
      ...
      if usage.uniform_texel_buffer || usage.storage_texel_buffer {
        output.alignment = align(output.alignment, limits.min_texel_buffer_offset_alignment() as usize);
      }

      if usage.storage_buffer {
        output.alignment = align(output.alignment, limits.min_storage_buffer_offset_alignment() as usize);
      }

      if usage.uniform_buffer {
        output.alignment = align(output.alignment, limits.min_uniform_buffer_offset_alignment() as usize);
      }
...
mod tests {
  ...
  fn panic_wrong_sparse_residency() {
    ...
    assert_should_panic!(
      "Can't enable sparse residency without enabling sparse \
       binding as well",
      {
        let _ = unsafe { UnsafeBuffer::new(device, 128, BufferUsage::all(), Sharing::Exclusive::<Empty<_>>, sparse) };
      }
    );
  }
  ...
  fn missing_feature_sparse_binding() {
    ...
    unsafe {
      match UnsafeBuffer::new(device, 128, BufferUsage::all(), Sharing::Exclusive::<Empty<_>>, sparse) {
