  ...
  A: MemoryPool,
{
  ...
  pub fn try_next(&self, data: T) -> Option<CpuBufferPoolSubbuffer<T, A>> {
    let mut mutex = self.current_buffer.lock().unwrap();
    self
      .try_next_impl(&mut mutex, iter::once(data))
      .map(|c| CpuBufferPoolSubbuffer {
        chunk: c
      })
      .ok()
  }
  ...
  fn reset_buf(&self, cur_buf_mutex: &mut MutexGuard<Option<Arc<ActualBuffer<A>>>>, capacity: usize) -> Result<(), DeviceMemoryAllocError> {
    unsafe {
      let (buffer, mem_reqs) = {
    ...
    I: ExactSizeIterator<Item = T>,
  {
    ...
    let (index, occupied_len, align_offset) = {
      let (tentative_index, tentative_len, tentative_align_offset) = {
        ...
        let align_bytes = cmp::max(
          if self.usage.uniform_buffer {
            self.device().physical_device().limits().min_uniform_buffer_offset_alignment() as usize
          } else {
            1
          },
          if self.usage.storage_buffer {
            self.device().physical_device().limits().min_storage_buffer_offset_alignment() as usize
          } else {
            1
          },
        );
      ...
      if tentative_index + tentative_len <= current_buffer.capacity && !chunks_in_use.iter().any(|c| {
        ...
        if requested_len <= current_buffer.capacity && !chunks_in_use.iter().any(|c| c.index < requested_len) {
          (0, requested_len, 0)
        } else {
  ...
  A: MemoryPool,
{
  fn clone(&self) -> CpuBufferPoolChunk<T, A> {
    ...
    chunk.num_cpu_accesses = chunk.num_cpu_accesses.checked_add(1).expect("Overflow in CPU accesses");
  ...
  A: MemoryPool,
{
  ...
  unsafe fn increase_gpu_lock(&self) {
    ...
    chunk.num_gpu_accesses = chunk.num_gpu_accesses.checked_add(1).expect("Overflow in GPU usages");
  }
