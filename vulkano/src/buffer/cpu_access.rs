...
impl<T> CpuAccessibleBuffer<T> {
  ...
  pub fn from_data(device: Arc<Device>, usage: BufferUsage, data: T) -> Result<Arc<CpuAccessibleBuffer<T>>, DeviceMemoryAllocError>
  where
    T: Content + 'static,
  {
    unsafe {
      let uninitialized = CpuAccessibleBuffer::raw(device, mem::size_of::<T>(), usage, iter::empty())?;
  ...
  #[inline]
  pub unsafe fn uninitialized(device: Arc<Device>, usage: BufferUsage) -> Result<Arc<CpuAccessibleBuffer<T>>, DeviceMemoryAllocError> {
...
impl<T> CpuAccessibleBuffer<[T]> {
  ...
  pub fn from_iter<I>(device: Arc<Device>, usage: BufferUsage, data: I) -> Result<Arc<CpuAccessibleBuffer<[T]>>, DeviceMemoryAllocError>
  where
    I: ExactSizeIterator<Item = T>,
    T: Content + 'static,
  {
    unsafe {
      let uninitialized = CpuAccessibleBuffer::uninitialized_array(device, data.len(), usage)?;
  ...
  A: MemoryPoolAlloc,
{
  ...
  pub fn read(&self) -> Result<ReadLock<T>, ReadLockError> {
    ...
    if let CurrentGpuAccess::Exclusive {
      ..
    } = *lock
    {
  ...
  pub fn write(&self) -> Result<WriteLock<T>, WriteLockError> {
    ...
    match *lock {
      CurrentGpuAccess::NonExclusive {
        ref num,
      }
        if num.load(Ordering::SeqCst) == 0 =>
      {
        ()
      }
  ...
  T: 'static + Send + Sync,
{
  ...
  fn try_gpu_lock(&self, exclusive_access: bool, _: &Queue) -> Result<(), AccessError> {
    if exclusive_access {
      ...
      *lock = CurrentGpuAccess::Exclusive {
        num: 1
      };
      Ok(())
    } else {
  ...
  unsafe fn increase_gpu_lock(&self) {
    // First, handle if we have a non-exclusive access.
    {
      ...
      if let CurrentGpuAccess::NonExclusive {
        ref num,
      } = *read_lock
      {
...
impl error::Error for ReadLockError {
  ...
  fn description(&self) -> &str {
    match *self {
      ReadLockError::CpuWriteLocked => "the buffer is already locked for write mode by the CPU",
      ReadLockError::GpuWriteLocked => "the buffer is already locked for write mode by the GPU",
    }
  }
}
