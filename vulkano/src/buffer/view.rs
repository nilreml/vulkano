  ...
  B: BufferAccess,
{
  ...
  pub unsafe fn unchecked(org_buffer: B, format: F) -> Result<BufferView<F, B>, BufferViewCreationError>
  where
    B: BufferAccess,
    F: FormatDesc + 'static,
  {
    let (view, format_props) = {
      ...
      if (offset % device.physical_device().limits().min_texel_buffer_offset_alignment() as usize) != 0 {
      ...
      let format_props = {
        ...
        vk_i.GetPhysicalDeviceFormatProperties(device.physical_device().internal_object(), format as u32, &mut output);
      ...
      let mut output = mem::uninitialized();
      check_errors(vk.CreateBufferView(device.internal_object(), &infos, ptr::null(), &mut output))?;
      (output, format_props)
    };
  ...
  B: BufferAccess,
{
  ...
  fn drop(&mut self) {
    unsafe {
      ...
      vk.DestroyBufferView(self.buffer.inner().buffer.device().internal_object(), self.view, ptr::null());
    }
  }
}
...
impl error::Error for BufferViewCreationError {
  ...
  fn description(&self) -> &str {
    match *self {
      ...
      BufferViewCreationError::WrongBufferUsage => "the buffer is missing correct usage flags",
      ...
      BufferViewCreationError::UnsupportedFormat => "the requested format is not supported for this usage",
...
mod tests {
  ...
  fn create_uniform() {
    ...
    let (buffer, _) = ImmutableBuffer::<[[u8; 4]]>::from_iter((0..128).map(|_| [0; 4]), usage, queue.clone()).unwrap();
  ...
  fn create_storage_atomic() {
    ...
    let (buffer, _) = ImmutableBuffer::<[u32]>::from_iter((0..128).map(|_| 0), usage, queue.clone()).unwrap();
  ...
  fn wrong_usage() {
    ...
    let (buffer, _) = ImmutableBuffer::<[[u8; 4]]>::from_iter((0..128).map(|_| [0; 4]), BufferUsage::none(), queue.clone()).unwrap();
  ...
  fn unsupported_format() {
    ...
    let (buffer, _) = ImmutableBuffer::<[[f64; 4]]>::from_iter((0..128).map(|_| [0.0; 4]), usage, queue.clone()).unwrap();
