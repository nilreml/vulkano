...
impl<T: ?Sized> ImmutableBuffer<T> {
  ...
  {
    let source =
      CpuAccessibleBuffer::from_data(queue.device().clone(), BufferUsage::transfer_source(), data)?;
...
impl<T> ImmutableBuffer<[T]> {
  ...
  {
    let source =
      CpuAccessibleBuffer::from_iter(queue.device().clone(), BufferUsage::transfer_source(), data)?;
    ImmutableBuffer::from_buffer(source, usage, queue)
  }
...
mod tests {
  ...
  fn from_iter_working() {
    ...
    let (buffer, _) =
      ImmutableBuffer::from_iter((0..512u32).map(|n| n * 2), BufferUsage::all(), queue.clone()).unwrap();

    let destination =
      CpuAccessibleBuffer::from_iter(device.clone(), BufferUsage::all(), (0..512).map(|_| 0u32)).unwrap();
  ...
  fn read_uninitialized_forbidden() {
    ...
    let (buffer, _) =
      unsafe { ImmutableBuffer::<u32>::uninitialized(device.clone(), BufferUsage::all()).unwrap() };
