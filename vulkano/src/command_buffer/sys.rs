...
impl Kind<RenderPass<EmptySinglePassRenderPassDesc>, Framebuffer<RenderPass<EmptySinglePassRenderPassDesc>, ()>> {
  ...
  #[inline]
  pub fn primary() -> Kind<Arc<RenderPass<EmptySinglePassRenderPassDesc>>, Framebuffer<RenderPass<EmptySinglePassRenderPassDesc>, ()>> {
    Kind::Primary
  }
...
impl<P> UnsafeCommandBufferBuilder<P> {
  ...
  pub unsafe fn new<Pool, R, F, A>(pool: &Pool, kind: Kind<R, F>, flags: Flags) -> Result<UnsafeCommandBufferBuilder<P>, OomError>
  where
  ...
  ///
  pub unsafe fn already_allocated<R, F>(alloc: P, kind: Kind<R, F>, flags: Flags) -> Result<UnsafeCommandBufferBuilder<P>, OomError>
  where
    R: RenderPassAbstract,
    F: FramebufferAbstract,
    P: CommandPoolBuilderAlloc,
  {
    ...
    let (oqe, qf, ps) = match kind {
      Kind::Secondary {
        ...
        debug_assert!(ps == 0 || alloc.device().enabled_features().pipeline_statistics_query);
  ...
  #[inline]
  pub unsafe fn begin_render_pass<F, I>(&mut self, framebuffer: &F, subpass_contents: SubpassContents, clear_values: I)
  where
    ...
    Cp: ?Sized + ComputePipelineAbstract,
  {
    let vk = self.device().pointers();
    let cmd = self.internal_object();
    vk.CmdBindPipeline(cmd, vk::PIPELINE_BIND_POINT_COMPUTE, pipeline.inner().internal_object());
  }
  ...
  #[inline]
  pub unsafe fn bind_vertex_buffers(&mut self, first_binding: u32, params: UnsafeCommandBufferBuilderBindVertexBuffer) {
    ...
    R: Iterator<Item = UnsafeCommandBufferBuilderImageCopy>,
  {
    ...
    debug_assert!(
      source.format().is_compressed() || destination.format().is_compressed() || source.format().size() == destination.format().size()
    );

    // Depth/Stencil formats are required to match exactly.
    debug_assert!(!source.format().ty().is_depth_and_or_stencil() || source.format() == destination.format());

    debug_assert_eq!(source.samples(), destination.samples());
    let source = source.inner();
    debug_assert!(source.image.usage_transfer_source());
    debug_assert!(source_layout == ImageLayout::General || source_layout == ImageLayout::TransferSrcOptimal);

    let destination = destination.inner();
    debug_assert!(destination.image.usage_transfer_destination());
    debug_assert!(destination_layout == ImageLayout::General || destination_layout == ImageLayout::TransferDstOptimal);

    let regions: SmallVec<[_; 8]> = regions
      .filter_map(|copy| {
        // TODO: not everything is checked here
        debug_assert!(copy.source_base_array_layer + copy.layer_count <= source.num_layers as u32);
        debug_assert!(copy.destination_base_array_layer + copy.layer_count <= destination.num_layers as u32);
        ...
        Some(vk::ImageCopy {
          ...
          dstSubresource: vk::ImageSubresourceLayers {
            ...
            baseArrayLayer: copy.destination_base_array_layer + destination.first_layer as u32,
            layerCount:     copy.layer_count,
          },
    ...
    R: Iterator<Item = UnsafeCommandBufferBuilderImageBlit>,
  {
    ...
    debug_assert!((source.format().ty() == FormatTy::Uint) == (destination.format().ty() == FormatTy::Uint));
    debug_assert!((source.format().ty() == FormatTy::Sint) == (destination.format().ty() == FormatTy::Sint));
    debug_assert!(source.format() == destination.format() || !source.format().ty().is_depth_and_or_stencil());
  ...
  pub unsafe fn clear_color_image<I, R>(&mut self, image: &I, layout: ImageLayout, color: ClearValue, regions: R)
  where
    I: ?Sized + ImageAccess,
    R: Iterator<Item = UnsafeCommandBufferBuilderColorImageClear>,
  {
    debug_assert!(image.format().ty() == FormatTy::Float || image.format().ty() == FormatTy::Uint || image.format().ty() == FormatTy::Sint);
    ...
    let color = match color {
      ClearValue::Float(val) => vk::ClearColorValue {
        float32: val
      },
      ClearValue::Int(val) => vk::ClearColorValue {
        int32: val
      },
      ClearValue::Uint(val) => vk::ClearColorValue {
        uint32: val
      },
  ...
  /// usage of the command anyway.
  #[inline]
  pub unsafe fn copy_buffer_to_image<S, D, R>(&mut self, source: &S, destination: &D, destination_layout: ImageLayout, regions: R)
  where
    S: ?Sized + BufferAccess,
    D: ?Sized + ImageAccess,
  ...
  /// usage of the command anyway.
  #[inline]
  pub unsafe fn copy_image_to_buffer<S, D, R>(&mut self, source: &S, source_layout: ImageLayout, destination: &D, regions: R)
  where
    S: ?Sized + ImageAccess,
    D: ?Sized + BufferAccess,
    R: Iterator<Item = UnsafeCommandBufferBuilderBufferImageCopy>,
  ...
  #[inline]
  pub unsafe fn copy_query_pool_results(&mut self, queries: UnsafeQueriesRange, destination: &BufferAccess, stride: usize) {
    let destination = destination.inner();
    debug_assert!(destination.offset < destination.buffer.size());
    debug_assert!(destination.buffer.usage_transfer_destination());
  ...
  pub unsafe fn dispatch(&mut self, dimensions: [u32; 3]) {
    debug_assert!({
      ...
      dimensions[0] <= max_dims[0] && dimensions[1] <= max_dims[1] && dimensions[2] <= max_dims[2]
    });
  ...
  #[inline]
  pub unsafe fn draw_indexed(&mut self, index_count: u32, instance_count: u32, first_index: u32, vertex_offset: i32, first_instance: u32) {
    let vk = self.device().pointers();
    let cmd = self.internal_object();
    ...
    B: ?Sized + BufferAccess,
  {
    ...
    debug_assert!(draw_count == 0 || ((stride % 4) == 0) && stride as usize >= mem::size_of::<vk::DrawIndirectCommand>());
    ...
    B: ?Sized + BufferAccess,
  {
    ...
    vk.CmdFillBuffer(cmd, buffer_handle, offset as vk::DeviceSize, size as vk::DeviceSize, data);
  }
  ...
  #[inline]
  pub unsafe fn push_constants<Pl, D>(&mut self, pipeline_layout: &Pl, stages: ShaderStages, offset: u32, size: u32, data: &D)
  where
    Pl: ?Sized + PipelineLayoutAbstract,
    D: ?Sized,
  {
  ...
  pub unsafe fn reset_query_pool(&mut self, queries: UnsafeQueriesRange) {
    let vk = self.device().pointers();
    let cmd = self.internal_object();
    vk.CmdResetQueryPool(cmd, queries.pool().internal_object(), queries.first_index(), queries.count());
  }
  ...
  {
    let scissors = scissors.map(|v| v.clone().into_vulkan_rect()).collect::<SmallVec<[_; 16]>>();
    if scissors.is_empty() {
      return;
    }

    debug_assert!(scissors.iter().all(|s| s.offset.x >= 0 && s.offset.y >= 0));
    debug_assert!(scissors.iter().all(|s| s.extent.width < i32::max_value() as u32
    ...
    debug_assert!((first_scissor == 0 && scissors.len() == 1) || self.device().enabled_features().multi_viewport);
    debug_assert!({
  ...
  {
    let viewports = viewports.map(|v| v.clone().into_vulkan_viewport()).collect::<SmallVec<[_; 16]>>();
    if viewports.is_empty() {
      return;
    }

    debug_assert!((first_viewport == 0 && viewports.len() == 1) || self.device().enabled_features().multi_viewport);
    debug_assert!({
      let max = self.device().physical_device().limits().max_viewports();
  ...
  pub unsafe fn write_timestamp(&mut self, query: UnsafeQuery, stages: PipelineStages) {
    let vk = self.device().pointers();
    let cmd = self.internal_object();
    vk.CmdWriteTimestamp(cmd, stages.into_vulkan_bits(), query.pool().internal_object(), query.index());
  }
...
impl UnsafeCommandBufferBuilderPipelineBarrier {
  ...
  ///
  #[inline]
  pub unsafe fn add_execution_dependency(&mut self, source: PipelineStages, destination: PipelineStages, by_region: bool) {
    if !by_region {
