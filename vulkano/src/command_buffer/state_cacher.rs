...
mod tests {
  ...
  fn vb_caching_single() {
    ...
    const EMPTY: [i32; 0] = [];
    let buf = CpuAccessibleBuffer::from_data(device, BufferUsage::vertex_buffer(), EMPTY.iter()).unwrap();
  ...
  fn vb_caching_multi() {
    ...
    const EMPTY: [i32; 0] = [];
    let buf1 = CpuAccessibleBuffer::from_data(device.clone(), BufferUsage::vertex_buffer(), EMPTY.iter()).unwrap();
    let buf2 = CpuAccessibleBuffer::from_data(device.clone(), BufferUsage::vertex_buffer(), EMPTY.iter()).unwrap();
    let buf3 = CpuAccessibleBuffer::from_data(device, BufferUsage::vertex_buffer(), EMPTY.iter()).unwrap();
