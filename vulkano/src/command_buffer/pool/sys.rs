...
impl UnsafeCommandPool {
  ...
  pub fn new(device: Arc<Device>, queue_family: QueueFamily, transient: bool, reset_cb: bool) -> Result<UnsafeCommandPool, OomError> {
    ...
    let pool = unsafe {
      ...
      check_errors(vk.CreateCommandPool(device.internal_object(), &infos, ptr::null(), &mut output))?;
      output
    };
  ...
  pub fn alloc_command_buffers(&self, secondary: bool, count: usize) -> Result<UnsafeCommandPoolAllocIter, OomError> {
    ...
    unsafe {
      ...
      check_errors(vk.AllocateCommandBuffers(self.device.internal_object(), &infos, out.as_mut_ptr()))?;
...
impl error::Error for CommandPoolTrimError {
  ...
  fn description(&self) -> &str {
    match *self {
      CommandPoolTrimError::Maintenance1ExtensionNotEnabled => "the `KHR_maintenance1` extension was not enabled",
    }
  }
}
