  ...
  D: ?Sized + ImageAccess,
{
  ...
  if source.format().is_compressed() || destination.format().is_compressed() || (source.format().size() != destination.format().size()) {
  ...
  if destination_offset[0] < 0 || destination_offset[0] as u32 + extent[0] > destination_dimensions.width() {
    return Err(CheckCopyImageError::DestinationCoordinatesOutOfRange);
  }

  if destination_offset[1] < 0 || destination_offset[1] as u32 + extent[1] > destination_dimensions.height() {
    return Err(CheckCopyImageError::DestinationCoordinatesOutOfRange);
  }

  if destination_offset[2] < 0 || destination_offset[2] as u32 + extent[2] > destination_dimensions.depth() {
    return Err(CheckCopyImageError::DestinationCoordinatesOutOfRange);
  }
...
impl error::Error for CheckCopyImageError {
  ...
  fn description(&self) -> &str {
    match *self {
      CheckCopyImageError::MissingTransferSourceUsage => "the source is missing the transfer source usage",
      CheckCopyImageError::MissingTransferDestinationUsage => "the destination is missing the transfer destination usage",
      CheckCopyImageError::SampleCountMismatch => "the number of samples in the source and destination do not match",
      ...
      CheckCopyImageError::IncompatibleRangeForImageType => "the offsets or extent are incompatible with the image type",
    }
  }
}
