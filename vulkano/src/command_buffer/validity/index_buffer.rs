...
///
pub fn check_index_buffer<B, I>(device: &Device, buffer: &B) -> Result<CheckIndexBuffer, CheckIndexBufferError>
where
...
impl error::Error for CheckIndexBufferError {
  ...
  fn description(&self) -> &str {
    match *self {
      CheckIndexBufferError::BufferMissingUsage => "the index buffer usage must be enabled on the index buffer",
      CheckIndexBufferError::WrongAlignment => {
        "the sum of offset and the address of the range of VkDeviceMemory object that is \
         backing buffer, must be a multiple of the type indicated by indexType"
      }
      CheckIndexBufferError::UnsupportIndexType => "the type of the indices is not supported by the device",
    }
  }
}
...
mod tests {
  ...
  fn num_indices() {
    ...
    let buffer = CpuAccessibleBuffer::from_iter(device.clone(), BufferUsage::index_buffer(), 0..500u32).unwrap();
  ...
  fn missing_usage() {
    let (device, queue) = gfx_dev_and_queue!();
    let buffer = CpuAccessibleBuffer::from_iter(device.clone(), BufferUsage::vertex_buffer(), 0..500u32).unwrap();
