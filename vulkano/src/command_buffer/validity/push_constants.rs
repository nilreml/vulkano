...
pub fn check_push_constants_validity<Pl, Pc>(pipeline: &Pl, push_constants: &Pc) -> Result<(), CheckPushConstantsValidityError>
where
...
impl error::Error for CheckPushConstantsValidityError {
  ...
  fn description(&self) -> &str {
    match *self {
      CheckPushConstantsValidityError::IncompatiblePushConstants => "the push constants are incompatible with the pipeline layout",
    }
  }
}
