...
impl error::Error for CheckCopyBufferImageError {
  ...
  fn description(&self) -> &str {
    match *self {
      CheckCopyBufferImageError::SourceMissingTransferUsage => "the source buffer is missing the transfer source usage",
      CheckCopyBufferImageError::DestinationMissingTransferUsage => "the destination buffer is missing the transfer destination usage",
      CheckCopyBufferImageError::OverlappingRanges => "the source and destination are overlapping",
      CheckCopyBufferImageError::UnexpectedMultisampled => "the image must not be multisampled",
      CheckCopyBufferImageError::ImageCoordinatesOutOfRange => "the image coordinates are out of range",
      CheckCopyBufferImageError::WrongPixelType(_) => "the type of pixels in the buffer isn't compatible with the image format",
