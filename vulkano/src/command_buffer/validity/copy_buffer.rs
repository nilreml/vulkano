...
///
pub fn check_copy_buffer<S, D, T>(device: &Device, source: &S, destination: &D) -> Result<CheckCopyBuffer, CheckCopyBufferError>
where
...
impl error::Error for CheckCopyBufferError {
  ...
  fn description(&self) -> &str {
    match *self {
      CheckCopyBufferError::SourceMissingTransferUsage => "the source buffer is missing the transfer source usage",
      CheckCopyBufferError::DestinationMissingTransferUsage => "the destination buffer is missing the transfer destination usage",
