...
///
pub fn check_update_buffer<B, D>(device: &Device, buffer: &B, data: &D) -> Result<(), CheckUpdateBufferError>
where
...
impl error::Error for CheckUpdateBufferError {
  ...
  fn description(&self) -> &str {
    match *self {
      CheckUpdateBufferError::BufferMissingUsage => "the transfer destination usage must be enabled on the buffer",
      CheckUpdateBufferError::WrongAlignment => "the offset or size are not aligned to 4 bytes",
...
mod tests {
  ...
  fn missing_usage() {
    ...
    let buffer = CpuAccessibleBuffer::from_data(device.clone(), BufferUsage::vertex_buffer(), 0u32).unwrap();
  ...
  fn data_too_large() {
    let (device, queue) = gfx_dev_and_queue!();
    let buffer = CpuAccessibleBuffer::from_iter(device.clone(), BufferUsage::transfer_destination(), 0..65536).unwrap();
  ...
  fn data_just_large_enough() {
    let (device, queue) = gfx_dev_and_queue!();
    let buffer = CpuAccessibleBuffer::from_iter(device.clone(), BufferUsage::transfer_destination(), (0..100000).map(|_| 0)).unwrap();
  ...
  fn wrong_alignment() {
    let (device, queue) = gfx_dev_and_queue!();
    let buffer = CpuAccessibleBuffer::from_iter(device.clone(), BufferUsage::transfer_destination(), 0..100).unwrap();
