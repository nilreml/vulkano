...
pub unsafe trait CommandBuffer: DeviceOwned {
  ...
  #[inline]
  fn execute(self, queue: Arc<Queue>) -> Result<CommandBufferExecFuture<NowFuture, Self>, CommandBufferExecError>
  where
    Self: Sized + 'static,
  {
  ...
  /// Panics if the device of the command buffer is not the same as the device of the future.
  #[inline]
  fn execute_after<F>(self, future: F, queue: Arc<Queue>) -> Result<CommandBufferExecFuture<F, Self>, CommandBufferExecError>
  where
    Self: Sized + 'static,
    F: GpuFuture,
  {
  ...
  Cb: CommandBuffer,
{
  ...
  fn check_buffer_access(
    ...
    match self.command_buffer.check_buffer_access(buffer, exclusive, queue) {
      ...
      Err(AccessCheckError::Unknown) => self.previous.check_buffer_access(buffer, exclusive, queue),
    }
  }
  ...
  fn check_image_access(
    ...
    match self.command_buffer.check_image_access(image, layout, exclusive, queue) {
      Ok(v) => Ok(v),
      Err(AccessCheckError::Denied(err)) => Err(AccessCheckError::Denied(err)),
      Err(AccessCheckError::Unknown) => self.previous.check_image_access(image, layout, exclusive, queue),
    }
  }
}
