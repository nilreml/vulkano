...
impl<P> SyncCommandBufferBuilder<P> {
    ...
    I: Iterator<Item = ClearValue> + Send + Sync + 'static,
  {
      ...
      I: Iterator<Item = ClearValue>,
    {
      ...
      unsafe fn send(&mut self, out: &mut UnsafeCommandBufferBuilder<P>) {
        out.begin_render_pass(&self.framebuffer, self.subpass_contents, self.clear_values.take().unwrap());
      }
  ...
  #[inline]
  pub unsafe fn bind_index_buffer<B>(&mut self, buffer: B, index_ty: IndexType) -> Result<(), SyncCommandBufferBuilderError>
  where
    ...
    R: Iterator<Item = UnsafeCommandBufferBuilderColorImageClear> + Send + Sync + 'static,
  {
      ...
      R: Iterator<Item = UnsafeCommandBufferBuilderColorImageClear> + Send + Sync + 'static,
    {
      ...
      unsafe fn send(&mut self, out: &mut UnsafeCommandBufferBuilder<P>) {
        out.clear_color_image(self.image.as_ref().unwrap(), self.layout, self.color, self.regions.take().unwrap());
      }
  ...
  /// usage of the command anyway.
  #[inline]
  pub unsafe fn copy_buffer<S, D, R>(&mut self, source: S, destination: D, regions: R) -> Result<(), SyncCommandBufferBuilderError>
  where
  ...
  #[inline]
  pub unsafe fn dispatch_indirect<B>(&mut self, buffer: B) -> Result<(), SyncCommandBufferBuilderError>
  where
    B: BufferAccess + Send + Sync + 'static,
  {
    struct Cmd<B> {
  ...
  pub unsafe fn draw(&mut self, vertex_count: u32, instance_count: u32, first_vertex: u32, first_instance: u32) {
    ...
    impl<P> Command<P> for Cmd {
      ...
      unsafe fn send(&mut self, out: &mut UnsafeCommandBufferBuilder<P>) {
        out.draw(self.vertex_count, self.instance_count, self.first_vertex, self.first_instance);
      }
  ...
  #[inline]
  pub unsafe fn draw_indirect<B>(&mut self, buffer: B, draw_count: u32, stride: u32) -> Result<(), SyncCommandBufferBuilderError>
  where
    B: BufferAccess + Send + Sync + 'static,
  {
    struct Cmd<B> {
  ...
  #[inline]
  pub unsafe fn draw_indexed_indirect<B>(&mut self, buffer: B, draw_count: u32, stride: u32) -> Result<(), SyncCommandBufferBuilderError>
  where
    B: BufferAccess + Send + Sync + 'static,
  {
    struct Cmd<B> {
    ...
    D: ?Sized + Send + Sync + 'static,
  {
      ...
      Pl: PipelineLayoutAbstract + Send + Sync + 'static,
    {
      ...
      unsafe fn send(&mut self, out: &mut UnsafeCommandBufferBuilder<P>) {
        out.push_constants::<_, [u8]>(&self.pipeline_layout, self.stages, self.offset, self.size, &self.data);
      }
      ...
      for ds in self.inner.iter() {
        for img_num in 0..ds.num_images() {
          ...
          let layout = match desc.ty {
            DescriptorDescTy::CombinedImageSampler(_) => image_view.descriptor_set_combined_image_sampler_layout(),
