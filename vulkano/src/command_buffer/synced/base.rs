...
impl<P> BuilderKey<P> {
  ...
  fn into_cb_key(self, final_commands: Arc<Mutex<Vec<Box<FinalCommand + Send + Sync>>>>) -> CbKey<'static> {
    CbKey::Command {
...
impl<P> SyncCommandBufferBuilder<P> {
  ...
  pub unsafe fn new<Pool, R, F, A>(pool: &Pool, kind: Kind<R, F>, flags: Flags) -> Result<SyncCommandBufferBuilder<P>, OomError>
  where
      ...
      Entry::Occupied(entry) => {
        ...
        if exclusive || entry.exclusive || entry.current_layout != start_layout {
          ...
          unsafe {
            ...
            match resource_ty {
              KeyTy::Buffer => {
                let buf = commands_lock.commands[latest_command_id].buffer(resource_index);

                let b = &mut self.pending_barrier;
                b.add_buffer_memory_barrier(buf, entry.stages, entry.access, stages, access, true, None, 0, buf.size());
              }
  ...
  {
    let mut commands_lock = self.commands.lock().unwrap();
    debug_assert!(commands_lock.latest_render_pass_enter.is_none() || self.pending_barrier.is_empty());
    ...
    let final_resources_states: FnvHashMap<_, _> = {
      self
        .resources
        .into_iter()
        .map(|(resource, state)| (resource.into_cb_key(final_commands.clone()), state.finalize()))
        .collect()
    };
...
impl<'a> CbKey<'a> {
  #[inline]
  fn conflicts_buffer(&self, commands_lock: Option<&Vec<Box<FinalCommand + Send + Sync>>>, buf: &BufferAccess) -> bool {
    match *self {
      CbKey::Command {
        ref commands,
        command_id,
  ...
  #[inline]
  fn conflicts_image(&self, commands_lock: Option<&Vec<Box<FinalCommand + Send + Sync>>>, img: &ImageAccess) -> bool {
    match *self {
      CbKey::Command {
        ref commands,
        command_id,
...
impl<P> SyncCommandBuffer<P> {
  ...
  pub fn lock_submit(&self, future: &GpuFuture, queue: &Queue) -> Result<(), CommandBufferExecError> {
    let commands_lock = self.commands.lock().unwrap();
    ...
    for (key, entry) in self.resources.iter() {
      ...
      match resource_ty {
        ...
        KeyTy::Image => {
          ...
          let prev_err = match future.check_image_access(img, entry.initial_layout, entry.exclusive, queue) {
            Ok(_) => {
              unsafe {
