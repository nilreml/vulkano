...
impl AutoCommandBufferBuilder<StandardCommandPoolBuilder> {
  #[inline]
  pub fn new(device: Arc<Device>, queue_family: QueueFamily) -> Result<AutoCommandBufferBuilder<StandardCommandPoolBuilder>, OomError> {
  ...
  #[inline]
  pub fn primary(device: Arc<Device>, queue_family: QueueFamily) -> Result<AutoCommandBufferBuilder<StandardCommandPoolBuilder>, OomError> {
    AutoCommandBufferBuilder::with_flags(device, queue_family, Kind::primary(), Flags::None)
  }
  ...
  pub fn primary_one_time_submit(
    ...
    AutoCommandBufferBuilder::with_flags(device, queue_family, Kind::primary(), Flags::OneTimeSubmit)
  }
  ...
  pub fn primary_simultaneous_use(
    ...
    AutoCommandBufferBuilder::with_flags(device, queue_family, Kind::primary(), Flags::SimultaneousUse)
  }
  ...
  pub fn secondary_compute(
    ...
    let kind = Kind::secondary(KindOcclusionQuery::Forbidden, QueryPipelineStatisticFlags::none());
...
impl<P> AutoCommandBufferBuilder<P> {
  ...
  #[inline]
  fn ensure_inside_render_pass_secondary(&self) -> Result<(), AutoCommandBufferBuilderContextError> {
  ...
  #[inline]
  fn ensure_inside_render_pass_inline<Gp>(&self, pipeline: &Gp) -> Result<(), AutoCommandBufferBuilderContextError>
  where
  ...
  #[inline]
  pub fn begin_render_pass<F, C>(mut self, framebuffer: F, secondary: bool, clear_values: C) -> Result<Self, BeginRenderPassError>
  where
            ...
            if atch_desc.load == LoadOp::Clear {
              match clear_value {
                ClearValue::None => panic!(
                  "Bad ClearValue! index: {}, attachment index: {}, expected: {:?}, got: None",
                  clear_i,
                  atch_i,
                  atch_desc.format.ty()
                ),
                ClearValue::Float(_) => {
                  if atch_desc.format.ty() != FormatTy::Float {
                    panic!(
                      "Bad ClearValue! index: {}, attachment index: {}, expected: {:?}, got: Float",
                      clear_i,
                      atch_i,
                      atch_desc.format.ty()
                    );
                ...
                ClearValue::Int(_) => {
                  if atch_desc.format.ty() != FormatTy::Sint {
                    panic!(
                      "Bad ClearValue! index: {}, attachment index: {}, expected: {:?}, got: Int",
                      clear_i,
                      atch_i,
                      atch_desc.format.ty()
                    );
                ...
                ClearValue::Uint(_) => {
                  if atch_desc.format.ty() != FormatTy::Uint {
                    panic!(
                      "Bad ClearValue! index: {}, attachment index: {}, expected: {:?}, got: Uint",
                      clear_i,
                      atch_i,
                      atch_desc.format.ty()
                    );
                ...
                ClearValue::Depth(_) => {
                  if atch_desc.format.ty() != FormatTy::Depth {
                    panic!(
                      "Bad ClearValue! index: {}, attachment index: {}, expected: {:?}, got: Depth",
                      clear_i,
                      atch_i,
                      atch_desc.format.ty()
                    );
                ...
                ClearValue::Stencil(_) => {
                  if atch_desc.format.ty() != FormatTy::Stencil {
                    panic!(
                      "Bad ClearValue! index: {}, attachment index: {}, expected: {:?}, got: Stencil",
                      clear_i,
                      atch_i,
                      atch_desc.format.ty()
                    );
                ...
                ClearValue::DepthStencil(_) => {
                  if atch_desc.format.ty() != FormatTy::DepthStencil {
                    panic!(
                      "Bad ClearValue! index: {}, attachment index: {}, expected: {:?}, got: DepthStencil",
                      clear_i,
                      atch_i,
                      atch_desc.format.ty()
                    );
    ...
    unsafe {
      ...
      let copy = UnsafeCommandBufferBuilderImageCopy {
        ...
        aspect: UnsafeCommandBufferBuilderImageAspect {
          ...
          stencil: !source.has_color() && source.has_stencil() && destination.has_stencil(),
        },
        source_mip_level,
        destination_mip_level,
        source_base_array_layer,
  ...
  ///
  pub fn clear_color_image<I>(self, image: I, color: ClearValue) -> Result<Self, ClearColorImageError>
  where
    ...
    I: ImageAccess + Send + Sync + 'static,
  {
    unsafe {
      ...
      self.ensure_outside_render_pass()?;
      check_clear_color_image(self.device(), &image, first_layer, num_layers, first_mipmap, num_mipmaps)?;
      ...
      self
        .inner
        .clear_color_image(image, ImageLayout::TransferDstOptimal, color, iter::once(region))?;
      Ok(self)
    }
  }
  ...
  pub fn copy_buffer_to_image<S, D, Px>(self, source: S, destination: D) -> Result<Self, CopyBufferImageError>
  where
  ...
  pub fn copy_image_to_buffer<S, D, Px>(self, source: S, destination: D) -> Result<Self, CopyBufferImageError>
  where
    S: ImageAccess + Send + Sync + 'static,
  ...
  #[inline]
  pub fn dispatch<Cp, S, Pc>(mut self, dimensions: [u32; 3], pipeline: Cp, sets: S, constants: Pc) -> Result<Self, DispatchError>
  where
  ...
  #[inline]
  pub fn draw<V, Gp, S, Pc>(mut self, pipeline: Gp, dynamic: &DynamicState, vertices: V, sets: S, constants: Pc) -> Result<Self, DrawError>
  where
    ...
    I: Index + 'static,
  {
    unsafe {
      ...
      if let StateCacherOutcome::NeedChange = self.state_cacher.bind_index_buffer(&index_buffer, I::ty()) {
      ...
      self
        .inner
        .draw_indexed(ib_infos.num_indices as u32, vb_infos.instance_count as u32, 0, 0, 0);
      Ok(self)
    }
  }
    ...
    Ib: BufferAccess + TypedBufferAccess<Content = [DrawIndirectCommand]> + Send + Sync + 'static,
  {
    unsafe {
      ...
      self
        .inner
        .draw_indirect(indirect_buffer, draw_count, mem::size_of::<DrawIndirectCommand>() as u32)?;
      Ok(self)
    }
  }
  ...
  pub unsafe fn execute_commands<C>(mut self, command_buffer: C) -> Result<Self, ExecuteCommandsError>
  where
  ...
  #[inline]
  pub fn next_subpass(mut self, secondary: bool) -> Result<Self, AutoCommandBufferBuilderContextError> {
    unsafe {
      if self.secondary_cb {
        return Err(AutoCommandBufferBuilderContextError::ForbiddenInSecondary);
      }
  ...
  Pl: PipelineLayoutAbstract + Send + Sync + Clone + 'static,
{
  for num_range in 0..pipeline.num_push_constants_ranges() {
    ...
    destination.push_constants::<_, [u8]>(pipeline.clone(), range.stages, range.offset as u32, range.size as u32, data);
  }
}
...
impl error::Error for AutoCommandBufferBuilderContextError {
  ...
  fn description(&self) -> &str {
    match *self {
      AutoCommandBufferBuilderContextError::ForbiddenInSecondary => "operation forbidden in a secondary command buffer",
      AutoCommandBufferBuilderContextError::ForbiddenInsideRenderPass => "operation forbidden inside of a render pass",
      AutoCommandBufferBuilderContextError::ForbiddenOutsideRenderPass => "operation forbidden outside of a render pass",
      AutoCommandBufferBuilderContextError::NotSupportedByQueueFamily => "the queue family doesn't allow this operation",
