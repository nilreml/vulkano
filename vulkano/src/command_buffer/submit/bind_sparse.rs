...
impl<'a> SubmitBindSparseBuilder<'a> {
  ...
  /// error.
  #[inline]
  pub fn merge(&mut self, other: SubmitBindSparseBuilder<'a>) -> Result<(), SubmitBindSparseBuilder<'a>> {
  ...
  pub fn submit(self, queue: &Queue) -> Result<(), SubmitBindSparseError> {
    unsafe {
      ...
      check_errors(vk.QueueBindSparse(*queue, bs_infos.len() as u32, bs_infos.as_ptr(), self.fence))?;
      Ok(())
    }
  }
}
...
impl<'a> SubmitBindSparseBufferBindBuilder<'a> {
  ...
  pub unsafe fn add_bind(&mut self, offset: usize, size: usize, memory: &DeviceMemory, memory_offset: usize) {
...
impl<'a> SubmitBindSparseImageOpaqueBindBuilder<'a> {
  ...
  pub unsafe fn add_bind(&mut self, offset: usize, size: usize, memory: &DeviceMemory, memory_offset: usize, bind_metadata: bool) {
    self.binds.push(vk::SparseMemoryBind {
      resourceOffset: offset as vk::DeviceSize,
      size:           size as vk::DeviceSize,
      memory:         memory.internal_object(),
