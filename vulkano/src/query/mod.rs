...
impl UnsafeQueryPool {
  ...
  pub fn new(device: Arc<Device>, ty: QueryType, num_slots: u32) -> Result<UnsafeQueryPool, QueryPoolCreationError> {
    ...
    let pool = unsafe {
      ...
      check_errors(vk.CreateQueryPool(device.internal_object(), &infos, ptr::null(), &mut output))?;
      output
    };
