...
{
  let attachment_desc = render_pass.attachment_desc(attachment_num).expect("Attachment num out of range");
...
impl error::Error for IncompatibleRenderPassAttachmentError {
  ...
  fn description(&self) -> &str {
    match *self {
      ...
      IncompatibleRenderPassAttachmentError::NotIdentitySwizzled => "the image view does not use identity swizzling",
