  ...
  D: RenderPassDesc,
{
  ...
  /// mode.
  ///
  pub fn new(device: Arc<Device>, description: D) -> Result<RenderPass<D>, RenderPassCreationError> {
    ...
    let attachment_references = description
      ...
      .flat_map(|pass| {
        ...
        debug_assert!(
          pass.resolve_attachments.is_empty() || pass
            .resolve_attachments
            .iter()
        ...
        debug_assert!(
          pass
            ...
            .chain(pass.depth_stencil.clone().into_iter())
            .all(
              |(atch, layout)| if let Some(r) = pass.input_attachments.iter().find(|r| r.0 == atch) {
                r.1 == layout
              } else {
                true
              }
    ...
    let render_pass = unsafe {
      ...
      check_errors(vk.CreateRenderPass(device.internal_object(), &infos, ptr::null(), &mut output))?;
      output
    };
...
impl RenderPass<EmptySinglePassRenderPassDesc> {
  ...
  #[inline]
  pub fn empty_single_pass(device: Arc<Device>) -> Result<RenderPass<EmptySinglePassRenderPassDesc>, RenderPassCreationError> {
