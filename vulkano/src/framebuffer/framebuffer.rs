  ...
  A: fmt::Debug,
{
  ...
  fn fmt(&self, fmt: &mut fmt::Formatter) -> Result<(), fmt::Error> {
    fmt
      .debug_struct("FramebufferBuilder")
  ...
  A: AttachmentsList,
{
  ...
  pub fn add<T>(self, attachment: T) -> Result<FramebufferBuilder<Rp, (A, T)>, FramebufferCreationError>
  where
    T: ImageViewAccess,
  {
    ...
    let dimensions = match self.dimensions {
      ...
      FramebufferBuilderDimensions::AutoIdentical(Some(current)) => {
        if img_dims.width() != current[0] || img_dims.height() != current[1] || img_dims.array_layers() != current[2] {
      ...
      FramebufferBuilderDimensions::Specific(current) => {
        ...
        FramebufferBuilderDimensions::Specific([img_dims.width(), img_dims.height(), img_dims.array_layers()])
      }
    };
  ...
  pub fn build(self) -> Result<Framebuffer<Rp, A>, FramebufferCreationError> {
    ...
    let dimensions = match self.dimensions {
      ...
      FramebufferBuilderDimensions::AutoIdentical(None) | FramebufferBuilderDimensions::AutoSmaller(None) => {
    ...
    let framebuffer = unsafe {
      ...
      check_errors(vk.CreateFramebuffer(device.internal_object(), &infos, ptr::null(), &mut output))?;
      output
    };
...
impl error::Error for FramebufferCreationError {
  ...
  fn description(&self) -> &str {
    match *self {
      ...
      FramebufferCreationError::DimensionsTooLarge => "the dimensions of the framebuffer are too large",
      ...
      FramebufferCreationError::IncompatibleAttachment(_) => "one of the images cannot be used as the requested attachment",
      FramebufferCreationError::CantDetermineDimensions => "the framebuffer has no attachment and no dimension was specified",
    }
  }
...
mod tests {
  ...
  fn simple_create() {
    ...
    let image = AttachmentImage::new(device.clone(), [1024, 768], Format::R8G8B8A8Unorm).unwrap();
