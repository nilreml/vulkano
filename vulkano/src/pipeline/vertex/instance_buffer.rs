  ...
  fn definition(
    ...
    let attrib = {
      ...
      for e in interface.elements() {
        ...
        if !infos
          .ty
          .matches(infos.array_size, e.format, e.location.end - e.location.start)
        {
...
unsafe impl<V> VertexSource<Vec<Arc<BufferAccess + Send + Sync>>> for SingleInstanceBufferDefinition<V>
where
  V: Vertex,
{
  #[inline]
