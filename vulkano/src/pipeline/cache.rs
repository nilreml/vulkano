...
impl PipelineCache {
  ...
  unsafe fn new_impl(device: Arc<Device>, initial_data: Option<&[u8]>) -> Result<Arc<PipelineCache>, OomError> {
    ...
    let cache = {
      ...
      check_errors(vk.CreatePipelineCache(device.internal_object(), &infos, ptr::null(), &mut output))?;
      output
    };
  ...
  pub fn get_data(&self) -> Result<Vec<u8>, OomError> {
    unsafe {
      ...
      let mut num = 0;
      check_errors(vk.GetPipelineCacheData(self.device.internal_object(), self.cache, &mut num, ptr::null_mut()))?;

      let mut data: Vec<u8> = Vec::with_capacity(num as usize);
      check_errors(vk.GetPipelineCacheData(self.device.internal_object(), self.cache, &mut num, data.as_mut_ptr() as *mut _))?;
