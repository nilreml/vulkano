...
where
  ...
  Gs::InputDefinition:
    ShaderInterfaceDefMatch<Tes::OutputDefinition> + ShaderInterfaceDefMatch<Vs::OutputDefinition>,
  ...
  > {
    ...
    if let Some(ref tess) = self.tessellation {
      if let Some(ref gs) = self.geometry_shader {
        ...
        {
          return Err(GraphicsPipelineCreationError::VertexTessControlStagesMismatch(
            err,
          ));
        }
        if let Err(err) = tess
