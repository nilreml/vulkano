  ...
  Rp: RenderPassAbstract + RenderPassSubpassInterface<Fs::OutputDefinition>,
{
  ...
  pub fn with_auto_layout(
    ...
    if let Some(ref tess) = self.tessellation {
      if let Some(ref gs) = self.geometry_shader {
        ...
        if let Err(err) = gs.0.input().matches(tess.tessellation_evaluation_shader.0.output()) {
      ...
      if let Some(ref geometry_shader) = self.geometry_shader {
        if let Err(err) = geometry_shader.0.input().matches(self.vertex_shader.as_ref().unwrap().0.output()) {
          return Err(GraphicsPipelineCreationError::VertexGeometryStagesMismatch(err));
        }
        if let Err(err) = self.fragment_shader.as_ref().unwrap().0.input().matches(geometry_shader.0.output()) {
          return Err(GraphicsPipelineCreationError::GeometryFragmentStagesMismatch(err));
        }
    ...
    Pl: PipelineLayoutAbstract,
  {
    ...
    PipelineLayoutSuperset::ensure_superset_of(&pipeline_layout, self.vertex_shader.as_ref().unwrap().0.layout())?;
    PipelineLayoutSuperset::ensure_superset_of(&pipeline_layout, self.fragment_shader.as_ref().unwrap().0.layout())?;
    if let Some(ref geometry_shader) = self.geometry_shader {
      PipelineLayoutSuperset::ensure_superset_of(&pipeline_layout, geometry_shader.0.layout())?;
    }
    if let Some(ref tess) = self.tessellation {
      PipelineLayoutSuperset::ensure_superset_of(&pipeline_layout, tess.tessellation_control_shader.0.layout())?;
      PipelineLayoutSuperset::ensure_superset_of(&pipeline_layout, tess.tessellation_evaluation_shader.0.layout())?;
    }
    ...
    let (binding_descriptions, attribute_descriptions) = {
      let (buffers_iter, attribs_iter) = self.vertex_input.definition(self.vertex_shader.as_ref().unwrap().0.input())?;
    ...
    if binding_descriptions.len() > device.physical_device().limits().max_vertex_input_bindings() as usize {
    ...
    if attribute_descriptions.len() > device.physical_device().limits().max_vertex_input_attributes() as usize {
    ...
    if self.input_assembly.primitiveRestartEnable != vk::FALSE && !self.input_assembly_topology.supports_primitive_restart() {
    ...
    let tessellation = match self.input_assembly_topology {
      PrimitiveTopology::PatchList {
        ...
        if vertices_per_patch > device.physical_device().limits().max_tessellation_patch_size() {
    ...
    if self.raster.polygon_mode != PolygonMode::Fill && !device.enabled_features().fill_mode_non_solid {
    ...
    let rasterization = vk::PipelineRasterizationStateCreateInfo {
      ...
      rasterizerDiscardEnable: if self.raster.rasterizer_discard { vk::TRUE } else { vk::FALSE },
    ...
    if self.multisample.sampleShadingEnable != vk::FALSE {
      debug_assert!(self.multisample.minSampleShading >= 0.0 && self.multisample.minSampleShading <= 1.0);
    ...
    let depth_stencil = {
      ...
      if self.depth_stencil.depth_compare != Compare::Always && !self.render_pass.as_ref().unwrap().has_depth() {
        return Err(GraphicsPipelineCreationError::NoDepthAttachment);
      }

      if (!self.depth_stencil.stencil_front.always_keep() || !self.depth_stencil.stencil_back.always_keep())
      ...
      vk::PipelineDepthStencilStateCreateInfo {
        ...
        pNext:                 ptr::null(),
        flags:                 0, // reserved
        depthTestEnable:       if !self.depth_stencil.depth_write && self.depth_stencil.depth_compare == Compare::Always {
          vk::FALSE
        } else {
          vk::TRUE
        },
        ...
        stencilTestEnable:     if self.depth_stencil.stencil_front.always_keep() && self.depth_stencil.stencil_back.always_keep() {
          vk::FALSE
        } else {
          vk::TRUE
        },
    ...
    let blend_atch: SmallVec<[vk::PipelineColorBlendAttachmentState; 8]> = {
      ...
      match self.blend.attachments {
        AttachmentsBlend::Collective(blend) => (0..num_atch).map(|_| blend.clone().into_vulkan_state()).collect(),
    ...
    let pipeline = unsafe {
      let infos = vk::GraphicsPipelineCreateInfo {
        ...
        pTessellationState:  tessellation.as_ref().map(|t| t as *const _).unwrap_or(ptr::null()),
        pViewportState:      &viewport_info,
        pRasterizationState: &rasterization,
        pMultisampleState:   &self.multisample,
        pDepthStencilState:  &depth_stencil,
        pColorBlendState:    &blend,
        pDynamicState:       dynamic_states.as_ref().map(|s| s as *const _).unwrap_or(ptr::null()),
      ...
      check_errors(vk.CreateGraphicsPipelines(device.internal_object(), 0, 1, &infos, ptr::null(), &mut output))?;
      output
    };
  ...
  GraphicsPipelineBuilder<Vdef, Vs, Vss, Tcs, Tcss, Tes, Tess, Gs, Gss, Fs, Fss, Rp>
{
  ...
  #[inline]
  pub fn vertex_input<T>(self, vertex_input: T) -> GraphicsPipelineBuilder<T, Vs, Vss, Tcs, Tcss, Tes, Tess, Gs, Gss, Fs, Fss, Rp> {
    GraphicsPipelineBuilder {
