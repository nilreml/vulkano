...
/// swapchain.
pub fn present<F, W>(swapchain: Arc<Swapchain<W>>, before: F, queue: Arc<Queue>, index: usize) -> PresentFuture<F, W>
where
  F: GpuFuture,
{
...
impl<W> Swapchain<W> {
  ...
  fn new_inner(
    ...
    let swapchain = unsafe {
      ...
      check_errors(vk.CreateSwapchainKHR(device.internal_object(), &infos, ptr::null(), &mut output))?;
      output
    };

    let image_handles = unsafe {
      let mut num = 0;
      check_errors(vk.GetSwapchainImagesKHR(device.internal_object(), swapchain, &mut num, ptr::null_mut()))?;

      let mut images = Vec::with_capacity(num as usize);
      check_errors(vk.GetSwapchainImagesKHR(device.internal_object(), swapchain, &mut num, images.as_mut_ptr()))?;
    ...
    let images = image_handles
      ...
      .map(|image| unsafe {
        ...
        let img = UnsafeImage::from_raw(device.clone(), image, usage.to_usage_bits(), format, dims, 1, 1);
...
impl error::Error for SwapchainCreationError {
  ...
  fn description(&self) -> &str {
    match *self {
      ...
      SwapchainCreationError::SurfaceInUse => "the surface is already used by another swapchain",
      SwapchainCreationError::NativeWindowInUse => "the window is already in use by another API",
      SwapchainCreationError::MissingExtension => "the `VK_KHR_swapchain` extension was not enabled",
      SwapchainCreationError::OldSwapchainSurfaceMismatch => "surface mismatch between old and new swapchain",
      SwapchainCreationError::OldSwapchainAlreadyUsed => "old swapchain has already been used to recreate a new one",
      SwapchainCreationError::UnsupportedMinImagesCount => "the requested number of swapchain images is not supported by the surface",
      SwapchainCreationError::UnsupportedMaxImagesCount => "the requested number of swapchain images is not supported by the surface",
      SwapchainCreationError::UnsupportedFormat => "the requested image format is not supported by the surface",
      SwapchainCreationError::UnsupportedDimensions => "the requested dimensions are not supported by the surface",
      SwapchainCreationError::UnsupportedArrayLayers => "the requested array layers count is not supported by the surface",
      SwapchainCreationError::UnsupportedUsageFlags => "the requested image usage is not supported by the surface",
      SwapchainCreationError::UnsupportedSurfaceTransform => "the requested surface transform is not supported by the surface",
      SwapchainCreationError::UnsupportedCompositeAlpha => "the requested composite alpha is not supported by the surface",
      SwapchainCreationError::UnsupportedPresentMode => "the requested present mode is not supported by the surface",
    }
  }
...
unsafe impl<W> GpuFuture for SwapchainAcquireFuture<W> {
  ...
  fn check_image_access(
    ...
    if self.swapchain.images[self.image_id].undefined_layout.load(Ordering::Relaxed) && layout != ImageLayout::Undefined {
