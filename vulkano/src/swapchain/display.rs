...
impl DisplayPlane {
  ...
  pub fn enumerate_raw(device: PhysicalDevice) -> Result<IntoIter<DisplayPlane>, OomError> {
    ...
    let num = unsafe {
      let mut num: u32 = 0;
      check_errors(vk.GetPhysicalDeviceDisplayPlanePropertiesKHR(device.internal_object(), &mut num, ptr::null_mut()))?;
      num
    };
    ...
    let planes: Vec<vk::DisplayPlanePropertiesKHR> = unsafe {
      ...
      let mut num = num;
      check_errors(vk.GetPhysicalDeviceDisplayPlanePropertiesKHR(device.internal_object(), &mut num, planes.as_mut_ptr()))?;
    ...
    Ok(
      planes
        ...
        .map(|(index, prop)| {
          let num = unsafe {
            let mut num: u32 = 0;
            check_errors(vk.GetDisplayPlaneSupportedDisplaysKHR(device.internal_object(), index as u32, &mut num, ptr::null_mut()))
          ...
          let supported_displays: Vec<vk::DisplayKHR> = unsafe {
            ...
            let mut num = num;
            check_errors(vk.GetDisplayPlaneSupportedDisplaysKHR(device.internal_object(), index as u32, &mut num, displays.as_mut_ptr()))
              .unwrap(); // TODO: shouldn't unwrap
...
impl Display {
  ...
  pub fn enumerate_raw(device: PhysicalDevice) -> Result<IntoIter<Display>, OomError> {
    ...
    let num = unsafe {
      let mut num = 0;
      check_errors(vk.GetPhysicalDeviceDisplayPropertiesKHR(device.internal_object(), &mut num, ptr::null_mut()))?;
      num
    };
    ...
    let displays: Vec<vk::DisplayPropertiesKHR> = unsafe {
      let mut displays = Vec::with_capacity(num as usize);
      let mut num = num;
      check_errors(vk.GetPhysicalDeviceDisplayPropertiesKHR(device.internal_object(), &mut num, displays.as_mut_ptr()))?;
      displays.set_len(num as usize);
      displays
    };
