...
unsafe impl DescriptorPool for LocalPool {
  ...
  fn alloc(&mut self, layout: &UnsafeDescriptorSetLayout) -> Result<Self::Alloc, OomError> {
    loop {
      ...
      let mut new_pool = UnsafeDescriptorPool::new(self.device.clone(), &count, self.next_capacity, false)?;
      let alloc = unsafe {
  ...
  L: PipelineLayoutAbstract,
{
  ...
  pub fn add_sampled_image<T>(
    ...
    FixedSizeDescriptorSetBuilder<'a, L, ((R, PersistentDescriptorSetImg<T>), PersistentDescriptorSetSampler)>,
  ...
  L: PipelineLayoutAbstract,
{
  ...
  pub fn add_sampled_image<T>(
    ...
    FixedSizeDescriptorSetBuilderArray<'a, L, ((R, PersistentDescriptorSetImg<T>), PersistentDescriptorSetSampler)>,
    PersistentDescriptorSetError,
  >
  where
    T: ImageViewAccess,
