  ...
  L: PipelineLayoutAbstract,
{
  ...
  ///
  pub fn build_with_pool<P>(self, pool: &mut P) -> Result<PersistentDescriptorSet<L, R, P::Alloc>, PersistentDescriptorSetBuildError>
  where
  ...
  pub fn add_sampled_image<T>(
    ...
    PersistentDescriptorSetBuilder<L, ((R, PersistentDescriptorSetImg<T>), PersistentDescriptorSetSampler)>,
  ...
  L: PipelineLayoutAbstract,
{
      ...
      DescriptorDescTy::Buffer(ref buffer_desc) => {
        ...
        if buffer_desc.storage {
          if !buffer.inner().buffer.usage_storage_buffer() {
            return Err(PersistentDescriptorSetError::MissingBufferUsage(MissingBufferUsage::StorageBuffer));
          }

          unsafe { DescriptorWrite::storage_buffer(self.builder.binding_id as u32, self.array_element as u32, &buffer) }
        } else {
          if !buffer.inner().buffer.usage_uniform_buffer() {
            return Err(PersistentDescriptorSetError::MissingBufferUsage(MissingBufferUsage::UniformBuffer));
          }

          unsafe { DescriptorWrite::uniform_buffer(self.builder.binding_id as u32, self.array_element as u32, &buffer) }
        }
      }
      ref d => {
        return Err(PersistentDescriptorSetError::WrongDescriptorTy {
    ...
    self.builder.writes.push(match self.desc.ty {
      DescriptorDescTy::TexelBuffer {
        ...
        if storage {
          ...
          DescriptorWrite::storage_texel_buffer(self.builder.binding_id as u32, self.array_element as u32, view.view())
        } else {
          ...
          DescriptorWrite::uniform_texel_buffer(self.builder.binding_id as u32, self.array_element as u32, view.view())
        }
      }
      ref d => {
        return Err(PersistentDescriptorSetError::WrongDescriptorTy {
    ...
    self.builder.writes.push(match desc.ty {
      ...
      DescriptorDescTy::InputAttachment {
        ...
        if !image_view.parent().inner().image.usage_input_attachment() {
          return Err(PersistentDescriptorSetError::MissingImageUsage(MissingImageUsage::InputAttachment));
        }
  ...
  pub fn add_sampled_image<T>(
    ...
    PersistentDescriptorSetBuilderArray<L, ((R, PersistentDescriptorSetImg<T>), PersistentDescriptorSetSampler)>,
    PersistentDescriptorSetError,
  >
  where
    T: ImageViewAccess,
  {
    ...
    assert_eq!(self.builder.layout.device().internal_object(), sampler.device().internal_object());
  ...
  pub fn add_sampler(
    ...
    self.builder.writes.push(match desc.ty {
      DescriptorDescTy::Sampler => DescriptorWrite::sampler(self.builder.binding_id as u32, self.array_element as u32, &sampler),
      ty => {
        return Err(PersistentDescriptorSetError::WrongDescriptorTy {
          expected: ty.ty().unwrap(),
        });
...
fn image_match_desc<I>(image_view: &I, desc: &DescriptorImageDesc) -> Result<(), PersistentDescriptorSetError>
where
...
impl error::Error for PersistentDescriptorSetError {
  ...
  fn description(&self) -> &str {
    match *self {
      ...
      PersistentDescriptorSetError::EmptyExpected => "expected an empty descriptor but got something",
      PersistentDescriptorSetError::ArrayOutOfBounds => "tried to add too many elements to an array",
      PersistentDescriptorSetError::MissingArrayElements {
        ..
      } => "didn't fill all the elements of an array before leaving",
      PersistentDescriptorSetError::IncompatibleImageViewSampler => "the image view isn't compatible with the sampler",
      ...
      PersistentDescriptorSetError::ExpectedMultisampled => "expected a multisampled image, but got a single-sampled image",
      PersistentDescriptorSetError::UnexpectedMultisampled => "expected a single-sampled image, but got a multisampled image",
