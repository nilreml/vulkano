...
impl UnsafeDescriptorPool {
  ...
  pub fn new(
    ...
    let pool = unsafe {
      ...
      check_errors(vk.CreateDescriptorPool(device.internal_object(), &infos, ptr::null(), &mut output))?;
      output
    };
  ...
  unsafe fn free_impl(&mut self, sets: &SmallVec<[vk::DescriptorSet; 8]>) -> Result<(), OomError> {
    let vk = self.device.pointers();
    check_errors(vk.FreeDescriptorSets(self.device.internal_object(), self.pool, sets.len() as u32, sets.as_ptr()))?;
    Ok(())
  }
  ...
  pub unsafe fn reset(&mut self) -> Result<(), OomError> {
    let vk = self.device.pointers();
    check_errors(vk.ResetDescriptorPool(self.device.internal_object(), self.pool, 0 /* reserved flags */))?;
    Ok(())
  }
}
...
impl fmt::Debug for UnsafeDescriptorPool {
  fn fmt(&self, fmt: &mut fmt::Formatter) -> Result<(), fmt::Error> {
    fmt
      .debug_struct("UnsafeDescriptorPool")
      .field("raw", &self.pool)
...
impl error::Error for DescriptorPoolAllocError {
  ...
  fn description(&self) -> &str {
    match *self {
      ...
      DescriptorPoolAllocError::OutOfPoolMemory => "there is no more space available in the descriptor pool",
    }
  }
}
    ...
    I: Iterator<Item = DescriptorWrite>,
  {
    ...
    for indiv_write in writes {
      ...
      for elem in indiv_write.inner.iter() {
        match *elem {
          DescriptorWriteInner::UniformBuffer(buffer, offset, size) | DescriptorWriteInner::DynamicUniformBuffer(buffer, offset, size) => {
          ...
          DescriptorWriteInner::StorageBuffer(buffer, offset, size) | DescriptorWriteInner::DynamicStorageBuffer(buffer, offset, size) => {
            buffer_descriptors.push(vk::DescriptorBufferInfo {
              buffer: buffer,
              offset: offset as u64,
              range:  size as u64,
          ...
          DescriptorWriteInner::UniformTexelBuffer(view) | DescriptorWriteInner::StorageTexelBuffer(view) => {
...
impl DescriptorWrite {
  ...
  #[inline]
  pub fn uniform_texel_buffer<'a, F, B>(binding: u32, array_element: u32, view: &BufferView<F, B>) -> DescriptorWrite
  where
    B: BufferAccess,
  {
  ...
  #[inline]
  pub fn storage_texel_buffer<'a, F, B>(binding: u32, array_element: u32, view: &BufferView<F, B>) -> DescriptorWrite
  where
    B: BufferAccess,
  {
    ...
    B: BufferAccess,
  {
    ...
    debug_assert_eq!(
      offset % buffer.device().physical_device().limits().min_uniform_buffer_offset_alignment() as usize,
      0
    );
    debug_assert!(size <= buffer.device().physical_device().limits().max_uniform_buffer_range() as usize);

    DescriptorWrite {
      binding:             binding,
      first_array_element: array_element,
      inner:               smallvec!({ DescriptorWriteInner::UniformBuffer(buffer.internal_object(), offset, size) }),
    }
  }
    ...
    B: BufferAccess,
  {
    ...
    debug_assert_eq!(
      offset % buffer.device().physical_device().limits().min_storage_buffer_offset_alignment() as usize,
      0
    );
    debug_assert!(size <= buffer.device().physical_device().limits().max_storage_buffer_range() as usize);

    DescriptorWrite {
      binding:             binding,
      first_array_element: array_element,
      inner:               smallvec!({ DescriptorWriteInner::StorageBuffer(buffer.internal_object(), offset, size) }),
    }
  }
