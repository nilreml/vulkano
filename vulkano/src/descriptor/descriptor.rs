...
impl DescriptorDesc {
  ...
  // TODO: add example
  #[inline]
  pub fn is_superset_of(&self, other: &DescriptorDesc) -> Result<(), DescriptorDescSupersetError> {
  ...
  pub fn pipeline_stages_and_access(&self) -> (PipelineStages, AccessFlagBits) {
    ...
    let access = match self.ty {
      ...
      DescriptorDescTy::CombinedImageSampler(_) | DescriptorDescTy::Image(_) => AccessFlagBits {
        shader_read: true,
...
impl DescriptorDescTy {
  ...
  // TODO: add example
  #[inline]
  pub fn is_superset_of(&self, other: &DescriptorDescTy) -> Result<(), DescriptorDescSupersetError> {
    match (self, other) {
...
impl DescriptorImageDescDimensions {
  ...
  pub fn from_dimensions(dims: Dimensions) -> DescriptorImageDescDimensions {
    match dims {
      Dimensions::Dim1d {
        ..
      } => DescriptorImageDescDimensions::OneDimensional,
      ...
      Dimensions::Dim3d {
        ..
      } => DescriptorImageDescDimensions::ThreeDimensional,
      Dimensions::Cubemap {
        ..
      } => DescriptorImageDescDimensions::Cube,
...
impl error::Error for DescriptorDescSupersetError {
  ...
  fn description(&self) -> &str {
    match *self {
      ...
      DescriptorDescSupersetError::TypeMismatch => "the descriptor type doesn't match the type of the other descriptor",
      DescriptorDescSupersetError::MutabilityRequired => "the descriptor is marked as read-only, but the other is not",
      DescriptorDescSupersetError::ShaderStagesNotSuperset => "the shader stages are not a superset of one another",
...
impl From<ShaderStagesSupersetError> for DescriptorDescSupersetError {
  ...
  fn from(err: ShaderStagesSupersetError) -> DescriptorDescSupersetError {
    match err {
      ShaderStagesSupersetError::NotSuperset => DescriptorDescSupersetError::ShaderStagesNotSuperset,
    }
  }
}
