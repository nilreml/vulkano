...
impl RuntimePipelineDesc {
    ...
    TPushConstsIter: IntoIterator<Item = PipelineLayoutDescPcRange>,
  {
    ...
    let push_constants: SmallVec<[PipelineLayoutDescPcRange; 6]> = push_constants.into_iter().collect();
...
mod tests {
  ...
  fn pc_conflict() {
    ...
    let r =
      RuntimePipelineDesc::new::<_, _, iter::Empty<Option<DescriptorDesc>>>(iter::empty(), iter::once(range1).chain(iter::once(range2)));
