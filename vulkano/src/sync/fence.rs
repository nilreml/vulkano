  ...
  D: SafeDeref<Target = Device>,
{
  ...
  fn alloc_impl(device: D, signaled: bool, must_put_in_pool: bool) -> Result<Fence<D>, OomError> {
    let fence = unsafe {
      ...
      check_errors(vk.CreateFence(device.internal_object(), &infos, ptr::null(), &mut output))?;
      output
    };
  ...
  pub fn ready(&self) -> Result<bool, OomError> {
    unsafe {
      ...
      let result = check_errors(vk.GetFenceStatus(self.device.internal_object(), self.fence))?;
      match result {
        Success::Success => {
  ...
  pub fn wait(&self, timeout: Option<Duration>) -> Result<(), FenceWaitError> {
    unsafe {
      ...
      let vk = self.device.pointers();
      let r = check_errors(vk.WaitForFences(self.device.internal_object(), 1, &self.fence, vk::TRUE, timeout_ns))?;
    ...
    D: 'a,
  {
    ...
    let r = if let Some(device) = device {
      unsafe {
        let vk = device.pointers();
        check_errors(vk.WaitForFences(device.internal_object(), fences.len() as u32, fences.as_ptr(), vk::TRUE, timeout_ns))?
      }
    } else {
      return Ok(());
    };
    ...
    D: 'a,
  {
    ...
    if let Some(device) = device {
      unsafe {
        let vk = device.pointers();
        check_errors(vk.ResetFences(device.internal_object(), fences.len() as u32, fences.as_ptr()))?;
      }
    }
    Ok(())
  }
