...
impl Event {
  ...
  fn alloc_impl(device: Arc<Device>, must_put_in_pool: bool) -> Result<Event, OomError> {
    let event = unsafe {
      ...
      let vk = device.pointers();
      check_errors(vk.CreateEvent(device.internal_object(), &INFOS, ptr::null(), &mut output))?;
      output
    };
  ...
  pub fn signaled(&self) -> Result<bool, OomError> {
    unsafe {
      ...
      let result = check_errors(vk.GetEventStatus(self.device.internal_object(), self.event))?;
      match result {
