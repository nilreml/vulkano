  ...
  F: GpuFuture,
{
  ...
  fn cleanup_finished_impl(&self) {
    ...
    match *state {
      FenceSignalFutureState::Flushed(ref mut prev, ref fence) => match fence.wait(Some(Duration::from_secs(0))) {
  ...
  fn flush_impl(&self, state: &mut MutexGuard<FenceSignalFutureState<F>>) -> Result<(), FlushError> {
    unsafe {
      ...
      let result = match previous.build_submission()? {
        ...
        SubmitAnyBuilder::QueuePresent(present) => {
          let intermediary_result = if partially_flushed { Ok(()) } else { present.submit(&queue) };
