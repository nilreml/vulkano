...
pub unsafe trait GpuFuture: DeviceOwned {
  ...
  /// > `CommandBuffer` trait.
  #[inline]
  fn then_execute<Cb>(self, queue: Arc<Queue>, command_buffer: Cb) -> Result<CommandBufferExecFuture<Self, Cb>, CommandBufferExecError>
  where
    Self: Sized,
  ...
  /// > `CommandBuffer` trait.
  #[inline]
  fn then_execute_same_queue<Cb>(self, command_buffer: Cb) -> Result<CommandBufferExecFuture<Self, Cb>, CommandBufferExecError>
  where
    Self: Sized,
    Cb: CommandBuffer + 'static,
  {
  ...
  #[inline]
  fn then_swapchain_present<W>(self, queue: Arc<Queue>, swapchain: Arc<Swapchain<W>>, image_index: usize) -> PresentFuture<Self, W>
  where
    Self: Sized,
  {
...
impl error::Error for AccessError {
  ...
  fn description(&self) -> &str {
    match *self {
      ...
      AccessError::AlreadyInUse => "the resource is already in use, and there is no tracking of concurrent usages",
      ...
      AccessError::BufferNotInitialized => "trying to use a buffer that still contains garbage data",
