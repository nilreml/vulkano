...
impl AccessFlagBits {
  ...
  pub fn is_compatible_with(&self, stages: &PipelineStages) -> bool {
    ...
    if (self.index_read || self.vertex_attribute_read) && !stages.vertex_input && !stages.all_graphics {
      return false;
    }
    ...
    if (self.color_attachment_read || self.color_attachment_write) && !stages.color_attachment_output && !stages.all_graphics {
      return false;
    }
